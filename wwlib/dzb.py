...
class DZB:
  ...
  def read(self, data):
    ...
    self.num_vertices = fs.read_u32(data, 0x00)
    self.vertex_list_offset = fs.read_u32(data, 0x04)
    
    self.num_faces = fs.read_u32(data, 0x08)
    self.face_list_offset = fs.read_u32(data, 0x0C)
    
    self.num_octree_blocks = fs.read_u32(data, 0x10)
    self.octree_block_list_offset = fs.read_u32(data, 0x14)
    self.num_octree_nodes = fs.read_u32(data, 0x18)
    self.octree_node_list_offset = fs.read_u32(data, 0x1C)
    
    self.num_groups = fs.read_u32(data, 0x20)
    self.group_list_offset = fs.read_u32(data, 0x24)
    
    self.num_properties = fs.read_u32(data, 0x28)
    self.property_list_offset = fs.read_u32(data, 0x2C)
    
    self.unknown_1 = fs.read_u32(data, 0x30)
  ...
  def save_changes(self):
    ...
    data = self.data
    fs.write_bytes(data, 0x00, b'\0'*0x34) # Header placeholder
    ...
    fs.align_data_to_nearest(data, 4, padding_bytes=b'\xFF')
    ...
    for group in self.groups:
      ...
      fs.write_str_with_null_byte(data, group.name_offset, group.name)
      group.save_changes()
      offset += len(group.name) + 1
    
    fs.align_data_to_nearest(data, 0x20, padding_bytes=b'\xFF')
    
    self.num_vertices = len(self.vertices)
    fs.write_u32(data, 0x00, self.num_vertices)
    fs.write_u32(data, 0x04, self.vertex_list_offset)
    
    self.num_faces = len(self.faces)
    fs.write_u32(data, 0x08, self.num_faces)
    fs.write_u32(data, 0x0C, self.face_list_offset)
    
    self.num_octree_blocks = len(self.octree_blocks)
    fs.write_u32(data, 0x10, self.num_octree_blocks)
    fs.write_u32(data, 0x14, self.octree_block_list_offset)
    
    self.num_octree_nodes = len(self.octree_nodes)
    fs.write_u32(data, 0x18, self.num_octree_nodes)
    fs.write_u32(data, 0x1C, self.octree_node_list_offset)
    
    self.num_groups = len(self.groups)
    fs.write_u32(data, 0x20, self.num_groups)
    fs.write_u32(data, 0x24, self.group_list_offset)
    
    self.num_properties = len(self.properties)
    fs.write_u32(data, 0x28, self.num_properties)
    fs.write_u32(data, 0x2C, self.property_list_offset)
    
    fs.write_u32(data, 0x30, self.unknown_1)
...
class Vertex:
  ...
  def read(self, offset):
    ...
    self.x_pos = fs.read_float(self.dzb_data, self.offset+0)
    self.y_pos = fs.read_float(self.dzb_data, self.offset+4)
    self.z_pos = fs.read_float(self.dzb_data, self.offset+8)
  
  def save_changes(self):
    fs.write_float(self.dzb_data, self.offset+0, self.x_pos)
    fs.write_float(self.dzb_data, self.offset+4, self.y_pos)
    fs.write_float(self.dzb_data, self.offset+8, self.z_pos)
...
class Face:
  ...
  def read(self, offset):
    ...
    self.vertex_1_index = fs.read_u16(self.dzb_data, self.offset+0)
    self.vertex_2_index = fs.read_u16(self.dzb_data, self.offset+2)
    self.vertex_3_index = fs.read_u16(self.dzb_data, self.offset+4)
    self.property_index = fs.read_u16(self.dzb_data, self.offset+6)
    self.group_index    = fs.read_u16(self.dzb_data, self.offset+8)
  ...
  def save_changes(self):
    fs.write_u16(self.dzb_data, self.offset+0, self.vertex_1_index)
    fs.write_u16(self.dzb_data, self.offset+2, self.vertex_2_index)
    fs.write_u16(self.dzb_data, self.offset+4, self.vertex_3_index)
    fs.write_u16(self.dzb_data, self.offset+6, self.property_index)
    fs.write_u16(self.dzb_data, self.offset+8, self.group_index)
...
class OctreeBlock:
  ...
  def read(self, offset):
    ...
    self.first_face_index = fs.read_u16(self.dzb_data, self.offset+0x00)
    
    # This will be populated by the DZB initialization function.
    self.faces = []
  
  def save_changes(self):
    fs.write_u16(self.dzb_data, self.offset+0x00, self.first_face_index)
...
class OctreeNode:
  ...
  def read(self, offset):
    ...
    flags = fs.read_u16(self.dzb_data, self.offset+0x00)
    self.is_leaf = (flags & 0x0001) != 0
    assert (flags & 0xFFFE) == 0x0100
    
    self.parent_node_index = fs.read_s16(self.dzb_data, self.offset+0x02)
    
    self.child_indexes = []
    for i in range(8):
      child_index = fs.read_s16(self.dzb_data, self.offset+0x04 + i*2)
  ...
  def save_changes(self):
    ...
    if self.is_leaf:
      flags |= 0x0001
    fs.write_u16(self.dzb_data, self.offset+0x00, flags)
    
    fs.write_s16(self.dzb_data, self.offset+0x02, self.parent_node_index)
    
    for i in range(8):
      child_index = self.child_indexes[i]
      fs.write_s16(self.dzb_data, self.offset+0x04 + i*2, child_index)
...
class Group:
  ...
  def read(self, offset):
    ...
    self.name_offset = fs.read_u32(self.dzb_data, self.offset+0x00)
    self.name = fs.read_str_until_null_character(self.dzb_data, self.name_offset)
    
    self.x_scale = fs.read_float(self.dzb_data, self.offset+0x04)
    self.y_scale = fs.read_float(self.dzb_data, self.offset+0x08)
    self.z_scale = fs.read_float(self.dzb_data, self.offset+0x0C)
    
    self.x_rot = fs.read_u16(self.dzb_data, self.offset+0x10)
    self.y_rot = fs.read_u16(self.dzb_data, self.offset+0x12)
    self.z_rot = fs.read_u16(self.dzb_data, self.offset+0x14)
    
    self.unknown_1 = fs.read_u16(self.dzb_data, self.offset+0x16) # Usually 0xFFFF, but sometimes 0xDCDC.
    
    self.x_translation = fs.read_float(self.dzb_data, self.offset+0x18)
    self.y_translation = fs.read_float(self.dzb_data, self.offset+0x1C)
    self.z_translation = fs.read_float(self.dzb_data, self.offset+0x20)
    
    self.parent_group_index       = fs.read_s16(self.dzb_data, self.offset+0x24)
    self.next_sibling_group_index = fs.read_s16(self.dzb_data, self.offset+0x26)
    self.first_child_group_index  = fs.read_s16(self.dzb_data, self.offset+0x28)
    
    # These will be populated by the DZB initialization function.
    self.parent_group = None
    self.children = None
    
    self.room_index             = fs.read_s16(self.dzb_data, self.offset+0x2A)
    self.first_vertex_index     = fs.read_u16(self.dzb_data, self.offset+0x2C)
    self.octree_root_node_index = fs.read_s16(self.dzb_data, self.offset+0x2E)
    
    bitfield = fs.read_u32(self.dzb_data, self.offset+0x30)
  ...
  def save_changes(self):
    fs.write_u32(self.dzb_data, self.offset+0x00, self.name_offset)
    
    fs.write_float(self.dzb_data, self.offset+0x04, self.x_scale)
    fs.write_float(self.dzb_data, self.offset+0x08, self.y_scale)
    fs.write_float(self.dzb_data, self.offset+0x0C, self.z_scale)
    
    fs.write_u16(self.dzb_data, self.offset+0x10, self.x_rot)
    fs.write_u16(self.dzb_data, self.offset+0x12, self.y_rot)
    fs.write_u16(self.dzb_data, self.offset+0x14, self.z_rot)
    
    fs.write_u16(self.dzb_data, self.offset+0x16, self.unknown_1)
    
    fs.write_float(self.dzb_data, self.offset+0x18, self.x_translation)
    fs.write_float(self.dzb_data, self.offset+0x1C, self.y_translation)
    fs.write_float(self.dzb_data, self.offset+0x20, self.z_translation)
    
    fs.write_s16(self.dzb_data, self.offset+0x24, self.parent_group_index)
    fs.write_s16(self.dzb_data, self.offset+0x26, self.next_sibling_group_index)
    fs.write_s16(self.dzb_data, self.offset+0x28, self.first_child_group_index)
    
    fs.write_s16(self.dzb_data, self.offset+0x2A, self.room_index)
    fs.write_u16(self.dzb_data, self.offset+0x2C, self.first_vertex_index)
    fs.write_s16(self.dzb_data, self.offset+0x2E, self.octree_root_node_index)
    ...
    fs.write_u32(self.dzb_data, self.offset+0x30, bitfield)
...
class Property:
  ...
  def read(self, offset):
    ...
    bitfield_1 = fs.read_u32(self.dzb_data, self.offset+0x00)
    ...
    bitfield_2 = fs.read_u32(self.dzb_data, self.offset+0x04)
    ...
    bitfield_3 = fs.read_u32(self.dzb_data, self.offset+0x08)
    ...
    bitfield_4 = fs.read_s32(self.dzb_data, self.offset+0x0C)
  ...
  def save_changes(self):
    ...
    fs.write_u32(self.dzb_data, self.offset+0x00, bitfield_1)
    ...
    fs.write_u32(self.dzb_data, self.offset+0x04, bitfield_2)
    ...
    fs.write_u32(self.dzb_data, self.offset+0x08, bitfield_3)
    ...
    fs.write_s32(self.dzb_data, self.offset+0x0C, bitfield_4)
