...
class EnemyRandomizer(BaseRandomizer):
  ...
  def _save(self):
    # TODO: This is a hack. _save is not intended to use RNG and should be deterministic, but the
    ...
    self.add_modify_and_replace_actors_for_enemy_rando()
    
    self.randomize_enemies_spawned_by_objects()
    
    self.update_loaded_particles(self.particles_to_load_for_each_jpc_index)
  
  
  #region Randomization
  def decide_on_enemy_pool_for_stage(self, stage_folder: str, enemy_locations: list[dict]):
    ...
    for category, enemies_logically_allowed in category_and_logic_combos_needed:
      enemies_allowed_for_combo = [
        ...
        if self.is_enemy_allowed_in_placement_category(enemy_data, category)
      ]
  ...
  def randomize_enemy_groups_for_stage(self, stage_folder, enemy_locations):
    ...
    enemy_pool_for_stage = self.decide_on_enemy_pool_for_stage(stage_folder, enemy_locations)
    for enemy_group in enemy_locations:
      room_attempts = 0
  ...
  def randomize_enemy_group(self, stage_folder, enemy_group, enemy_pool_for_stage):
    ...
    free_memory = self.get_free_memory_for_group(enemy_group)
    ...
    for enemy_location in enemy_group["Enemies"]:
      arc_name = enemy_location["Path"].split("/")[1]
      
      enemies_to_randomize_to_for_this_location = [
        data for data in enemy_pool_for_group
        if self.is_enemy_allowed_in_placement_category(data, enemy_location["Placement category"])
      ]
      ...
      memory_for_enemy = self.get_amount_of_memory_for_enemy(new_enemy_data, enemy_actor_names_already_placed_in_room)
  ...
  def save_changed_enemies_and_randomize_their_params(self):
    ...
    for enemy_location, new_enemy_data in self.done_enemy_locations:
      ...
      enemy, arc_name, dzx, layer = self.get_enemy_instance_by_path(path)
      ...
      original_enemy_type = self.get_enemy_data_for_actor(enemy)
      ...
      self.randomize_enemy_params(new_enemy_data, enemy, placement_category, dzx, layer)
      self.adjust_enemy(new_enemy_data, enemy, placement_category, dzx, layer)
      ...
      if stage_folder == "sea":
        dzr = self.rando.get_arc("files/res/Stage/sea/" + room_arc_name).get_file("room.dzr", DZx)
        dest_jpc_index = dzr.entries_by_type(FILI)[0].loaded_particle_bank
      else:
        dzs = self.rando.get_arc("files/res/Stage/" + stage_folder + "/Stage.arc").get_file("stage.dzs", DZx)
  ...
  def adjust_enemy(self, enemy_data, enemy, category, dzx, layer):
    ...
    if enemy.name == "magtail":
      ...
      closest_pot = min(pots_on_same_layer, key=lambda pot: self.distance_between_entities(enemy, pot))
  ...
  def add_modify_and_replace_actors_for_enemy_rando(self):
    ...
    for stage_folder, enemy_locations in self.enemy_locations.items():
      for enemy_group in enemy_locations:
        ...
        for actor_path in actor_paths:
          actor = self.get_actor_by_path(actor_path)
    ...
    for stage_folder, enemy_locations in self.enemy_locations.items():
      for enemy_group in enemy_locations:
        ...
        for existing_actor_data in existing_actor_datas:
          actor = self.get_actor_by_path(existing_actor_data["Path"])
    ...
    for stage_folder, enemy_locations in self.enemy_locations.items():
      for enemy_group in enemy_locations:
        ...
        for new_actor_data in new_actor_datas:
          dzx, _, layer = self.get_dzx_fourcc_and_layer_by_path(enemy_group["Enemies"][0]["Path"])
    ...
    for stage_folder, enemy_locations in self.enemy_locations.items():
      for enemy_group in enemy_locations:
        ...
        for actor_path in actor_paths:
          dzx, fourcc, layer = self.get_dzx_fourcc_and_layer_by_path(actor_path)
          actor = self.get_actor_by_path(actor_path)
  ...
  def randomize_enemies_spawned_by_objects(self):
    ...
    dzr = self.rando.get_arc("files/res/Stage/kaze/Room2.arc").get_file("room.dzr", DZx)
  ...
  def update_loaded_particles(self, particles_to_load_for_each_jpc_index):
    ...
    for dest_jpc_index, particle_ids in particles_to_load_for_each_jpc_index.items():
      for particle_id in particle_ids:
        ...
        dest_jpc = self.rando.get_jpc(dest_jpc_path)
        ...
        if particle_id in particle_and_textures_by_id:
          ...
          for i in range(255):
            ...
            if src_jpc_path.lower() not in self.rando.gcm.files_by_path_lowercase:
              continue
            src_jpc = self.rando.get_jpc(src_jpc_path)
  ...
  def print_all_enemy_params(self):
    ...
    for dzx, arc_path in stage_searcher.each_stage_and_room(self.rando):
    ...
    for dzx, arc_path in stage_searcher.each_stage_and_room(self):
      for layer in DZxLayer:
        ...
        enemies = [
          ...
          and self.get_enemy_data_for_actor(actor)["Allow randomizing from"] # Don't list unrandomizable enemies
        ]
        ...
        for enemy in enemies:
          defeat_reqs = self.get_enemy_data_for_actor(enemy)["Requirements to defeat"]
        ...
        for enemy in enemies:
          enemy_data = self.get_enemy_data_for_actor(enemy)
          
          placement_category = self.get_placement_category_for_vanilla_enemy_location(enemy_data, enemy)
